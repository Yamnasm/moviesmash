...
async def pick(ctx, log=True, colour=None, validation=False):
    ...
    if ctx.author.id in ongoing_users and not validation:
        logger.info(f"{ctx.author.name}#{ctx.author.discriminator} attempted to invoke .pick while active")
    ...
    if not colour:
        logger.debug(f".pick: generated new colour for {ctx.author.name}#{ctx.author.discriminator}")
        colour = discord.Color.from_rgb(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
    else:
        logger.debug(f".pick: {ctx.author.name}#{ctx.author.discriminator} already has a generated colour")

    check = lambda reaction, user: not user.bot and user == ctx.author and str(reaction.emoji) in ("⬅️", "➡️", "↪️", "🛑")

    choices = util.create_choice()

    logger.debug(f".pick ({ctx.author.name}#{ctx.author.discriminator}): profiling poster downloads...")
    ...
    logger.debug(f".pick ({ctx.author.name}#{ctx.author.discriminator}): stitching posters together...")
    data = util.convert_posters_to_single_image(poster1, poster2)

    logger.debug(f".pick ({ctx.author.name}#{ctx.author.discriminator}): uploading bytesio object to discord api...")
    file = discord.File(data, filename="posters.png")

    msg = await ctx.send(file=file, embed=embed)
    logger.debug(f".pick ({ctx.author.name}#{ctx.author.discriminator}): uploaded bytesio file to discord api!")

    end_time = time.time() - start_time
    logger.debug(f".pick ({ctx.author.name}#{ctx.author.discriminator}): profiling finished (the routine took {end_time:.2f} seconds)\n")
    ...
    if   response[0].emoji == "⬅️":
        ...
        logger.info(f".pick ({ctx.author.name}#{ctx.author.discriminator}): {choices[1]['name']} won against {choices[0]['name']}")
        ...
        logger.info(f".pick ({ctx.author.name}#{ctx.author.discriminator}): skipped a vote")
        await msg.delete()
        await pick(ctx, log=False, colour=colour, validation=True)

    elif response[0].emoji == "🛑":
        logger.info(f".pick ({ctx.author.name}#{ctx.author.discriminator}): stopped requesting choices")
